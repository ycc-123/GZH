var _aa = {};
_aa.checkAndNudgePoints = function (image, points) {
...
_aa.sampleGrid3 = function (image, dimension, transform) {
	...
	for (var y = 0; y < dimension; y++) {
		...
		_aa.checkAndNudgePoints(image, points);
		try {
			for (var x = 0; x < max; x += 2) {
...
_aa.sampleGridx = function (
	...
	return _aa.sampleGrid3(image, dimension, transform);
};
			...
			for (var dy = 0; dy < areaHeight; dy++) {
				for (var dx = 0; dx < areaWidth; dx++) {
					bitmap[areaWidth * ax + dx + (areaHeight * ay + dy) * qrcode.width] =
						grayScale[areaWidth * ax + dx + (areaHeight * ay + dy) * qrcode.width] < middle[ax][ay] ? true : false;
				}
			}
		}
	}

import encoder from './qrcode.js';
// import decoder from './llqrcode.js';
// function getScaleSize(image, max, setTo = {}) {
// 	var nheight = image.height;
// 	var nwidth = image.width;
// 	if (image.width * image.height > max) {
// 		var ir = image.width / image.height;
// 		nheight = Math.sqrt(max / ir);
// 		nwidth = ir * nheight;
// 	}
// 	setTo.width = nwidth;
// 	setTo.height = nheight;
// 	return setTo;
// }
// export default function Qrcode(canvas) {
let qrcode = {};
// qrcode.decodeSrc = function (src) {
// 	let context = canvas.getContext('2d');
// 	let image = new Image();
// 	return new Promise(function (resolve, reject) {
// 		image.onload = function () {
// 			getScaleSize(image, decoder.maxImgSize, canvas);
// 			decoder.width = canvas.width;
// 			decoder.height = canvas.height;
// 			context.drawImage(image, 0, 0, canvas.width, canvas.height);
// 			try {
// 				decoder.imagedata = context.getImageData(0, 0, decoder.width, decoder.height);
// 				resolve(decoder.process());
// 			} catch (e) {
// 				console.log(e);
// 				reject(e);
// 			}
...
// 	});
// };
qrcode.encode = function (value, options = {}) {
	if (typeof value == 'object') {
		options = value;
		value = options.value;
	}
	let qr = new encoder(options.typeNumber, options.errorCorrectLevel);
	if (value) qr.addData(value);
	qr.make();
	return qr;
};
qrcode.poster = function (canvas, width, height, actions, resources) {
	canvas.width = width;
